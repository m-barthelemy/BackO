...
namespace Node.DataProcessing{
    ...
    public class USNHelper : IDisposable   {
       ...
       /* public void EnumerateFilesAndFolders(string rootpath, Duplicati.Library.Utility.FilenameFilter filter, Duplicati.Library.Utility.Utility.EnumerationCallbackDelegate callback)
		...
		/*internal IEnumerable<KeyValuePair<string,Win32USN.USN_RECORD>> GetChangedRecords(string sourceFolder, long refUsn){
			// Populate records list
			GetChangedItems(refUsn);
            foreach (KeyValuePair<string, Win32USN.USN_RECORD> r in this.Records){
				if(r.Value.Usn >= refUsn && r.Key.StartsWith(sourceFolder, StringComparison.Ordinal))
					yield return r;
			}
        }*/
		
		/*internal IEnumerable<KeyValuePair<string,Win32USN.USN_RECORD>> GetChangedRecords(string sourceFolder, long lastUsn){
            const int ALLOCATED_MEMORY = 64 * 1024;
			//const int ALLOCATED_MEMORY = 16 * 1024;
			Console.WriteLine("GetChangedRecords() : 1 - lastUsn="+lastUsn);
           ...
           // List<KeyValuePair<string, Win32USN.USN_RECORD>> records = new List<KeyValuePair<string, Win32USN.USN_RECORD>>();
            ...
            try{
				...
				startParams.StartUsn = Math.Max(lastUsn, m_journal.LowestValidUsn);
                startParams.ReasonMask = Win32USN.USNReason.USN_REASON_ANY;
                startParams.ReturnOnlyOnClose = 0;
				//startParams.ReturnOnlyOnClose = false;
                startParams.Timeout = 0;
                startParams.BytesToWaitFor = 0;
				startParams.UsnJournalID = m_journal.UsnJournalID;
				 

				Console.WriteLine("GetChangedRecords() : 2");

				Console.WriteLine("BuildUSNTable() ");
				//BuildUSNTable(lastUsn);

				Console.WriteLine("end BuildUSNTable() : records ="+this.Records.Count);
				//foreach(var record in this.Records){
				//	if( ((Win32USN.USN_RECORD)record.Value).Usn > lastUsn){
				//		Console.Write ("@");
				//	Console.WriteLine("#"+record.Key+",reason="+(uint)((Win32USN.USN_RECORD)record.Value).Reason);
				//	}
				//}
				Console.WriteLine("end get records.");
                while (more){
					Console.WriteLine("GetChangedRecords() : inside loop");

					if (!Win32USN.DeviceIoControl(
							m_volumeHandle, 
					        Win32USN.EIOControlCode.FsctlReadUsnJournal,
                            ref startParams, 
					        (uint)Marshal.SizeOf(typeof(Win32USN.READ_USN_JOURNAL_DATA)),
                            ref allocatedMemory, 
					        ALLOCATED_MEMORY,
                            ref bytesRead,
					        IntPtr.Zero))
						{
						Console.WriteLine("GetChangedRecords() : 3");
                        int errorCode = Marshal.GetLastWin32Error();
						Console.WriteLine("GetChangedRecords() : 4");
                        ...
                        if (errorCode == Win32USN.ERROR_HANDLE_EOF || errorCode == Win32USN.ERROR_SUCCESS){
							Console.WriteLine("GetChangedRecords() : errorcode="+errorCode);
							more = false;
                           break;
						}
                        else{
                            Console.WriteLine("GetChangedRecords() :ERROR");
							throw new Win32Exception(errorCode);
							
						}
                    }
					Console.WriteLine("GetChangedRecords() : 5");
                    //startParams.StartUsn = ExtractUsnEntries(bytesRead, allocatedMemory, records, out more);
					foreach(KeyValuePair<string, Win32USN.USN_RECORD> entry in GetUsnEntry(bytesRead, allocatedMemory))
						yield return entry;
					Console.WriteLine("GetChangedRecords() : 6");
                } // end while
		...
		private IEnumerable<KeyValuePair<string, Win32USN.USN_RECORD>> GetUsnEntry(uint bytesRead, IntPtr startOffset){
            ...
            while (bytesLeft > USN_RECORD_SIZE){
                ...
                if (r.MajorVersion >= 2){
                    ...
                    yield return new KeyValuePair<string, Win32USN.USN_RECORD>(filename, r);
                }
        ...
        public List<string> GetRenamedFileSystemEntries(string sourceFolder, long lastUsn)
        {
            List<string> result = new List<string>();
            foreach (KeyValuePair<string, Win32USN.USN_RECORD> r in this.Records)
                if (r.Value.Usn >= lastUsn && ((r.Value.Reason & Win32USN.USNReason.USN_REASON_RENAME_NEW_NAME) != 0) )
                    result.Add(r.Key);
        ...
        internal List<KeyValuePair<string, Win32USN.USN_RECORD>> GetChangedItems(long lastUsn)
        {
			bool more = true;
			//int usnsize = (int)Marshal.SizeOf(typeof(long));
           /* const int ALLOCATED_MEMORY = 64 * 1024;
            ...
            try
            {
				...
				startParams.StartUsn = 0; //lastUsn;
				startParams.ReasonMask = Win32USN.USNReason.USN_REASON_ANY;
                //startParams.ReturnOnlyOnClose = 0;
				//startParams.ReturnOnlyOnClose = false;
                startParams.Timeout = 0;
                startParams.BytesToWaitFor = 0;
				startParams.UsnJournalID = (ulong)m_journal.UsnJournalID;
				Console.WriteLine ("GetChangedItems() : before loop");*/
		try{
			int pbDataSize = sizeof(UInt64) * 0x4000;
                ...
                rujd.UsnJournalId = (ulong)m_journal.UsnJournalID;
               ...
               /* while (more)
                {
                    ...
                    if (!Win32USN.DeviceIoControl(m_volumeHandle, Win32USN.EIOControlCode.FsctlReadUsnJournal,
                             ref startParams, (uint)Marshal.SizeOf(typeof(Win32USN.READ_USN_JOURNAL_DATA)),
                                ref allocatedMemory, ALLOCATED_MEMORY,
                                ref bytesRead, IntPtr.Zero))

                    {*/
                        int errorCode = Marshal.GetLastWin32Error();
						Console.WriteLine ("GetChangedItems() : errorcode="+errorCode+", bytesRead="+outBytesReturned);
                        //If we get no error or EOF the enumeration is completed
                        if (errorCode == Win32USN.ERROR_HANDLE_EOF || errorCode == Win32USN.ERROR_SUCCESS)
                            break;
                        else
					...
					Console.WriteLine ("GetChangedItems() : EVICEOONTROL5° CALL IS OK");
                    startParams.StartUsn = ExtractUsnEntries(bytesRead, allocatedMemory, records, out more);
                }
				...
				Console.WriteLine ("GetChangedItems() : Error : "+e.ToString());
				throw;
			}
            finally
            {
