...
using System.Collections.Generic;
	using System.Text;
namespace Node.DataProcessing.Utils{
	...
	// HELP at http://www.codeproject.com/KB/recipes/ManagedQLZ.aspx
	...
	public class QuickLZ{
	    ...
	    private unsafe static void write_header(byte[] dst, int level, bool compressible, int size_compressed, int size_decompressed){
		...
		public unsafe static int Compress(string inFile, string outFile){
			byte[] inFileBytes = File.ReadAllBytes(inFile);
			byte[] outFileBytes = Compress(inFileBytes, 1);
			System.IO.File.WriteAllBytes(outFile, outFileBytes);
		
			return outFileBytes.Length;
		}
		
		public unsafe static void Decompress(string inFile, string outFile){
			System.IO.File.WriteAllBytes(outFile, Decompress(File.ReadAllBytes(inFile)));	
		}
		
	    public unsafe static byte[] Compress(byte[] source, int level){
	        int src = 0;
	    ...
	    public unsafe static byte[] Decompress(byte[] source){
	        int level;
