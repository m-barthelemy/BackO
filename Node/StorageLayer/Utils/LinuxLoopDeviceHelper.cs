...
namespace Node.StorageLayer {
	...
	//Using great linux Pinvoke plumbery from http://stackoverflow.com/questions/10820311/c-c-c-howto-do-mount-a/11061630#11061630
	// loop partitions scanning LO_FLAGS_PARTSCAN needs kernel >= 3.2.
	// loop device containing partitions need kernel >= 2.6.26 (anyway we depend on LO_FLAGS_PARTSCAN, so >= 3.2)
	public class LinuxLoopDeviceHelper {
        ...
        */
        ...
        // umount("/mnt/testdisk");
        public bool Umount(string strMountPoint){
            int status = UnsafeNativeMethods.umount(strMountPoint);
            if (status != 0)
                throw new Exception("Error unmounting, Unmount status: " + status.ToString());
            else
                return true;
        ...
        private string find_unused_loop_device(){
            string dev;
            int fd;
                ...
                if (System.Convert.ToBoolean(Mono.Unix.Native.Syscall.stat(dev, out statbuf)) == (false && S_ISBLK((int)statbuf.st_mode)))
                {
                    ...
                    if ((fd = Mono.Unix.Native.Syscall.open(dev, Mono.Unix.Native.OpenFlags.O_RDONLY)) >= 0){
                        ...
                        if (UnsafeNativeMethods.ioctl(fd, LOOP_GET_STATUS64, ref lo64) == 0){
                            if (Mono.Unix.Native.Syscall.GetLastError() == Mono.Unix.Native.Errno.ENXIO){   // probably free
        ...
        private int set_loop(string device, string file, int offset, ref int loopro){
            loop_info loopinfo = new loop_info();
            int fd = 0, ffd = 0;
            ...
            {
                throw new Exception("Error opening file '" + file+"' : "+ffd);
            } // End if


            if ((fd = Mono.Unix.Native.Syscall.open(device, mode)) < 0){
                Mono.Unix.Native.Syscall.close(ffd);
                throw new Exception("Error opening device '" + device+"' : "+fd);
            }
			...
			loopinfo.lo_flags = (int)LoopFlags.LO_FLAGS_PARTSCAN;
			int retCode = UnsafeNativeMethods.ioctl(fd, LOOP_SET_FD, ffd);
            if (retCode < 0){
                Mono.Unix.Native.Syscall.close(fd);
                Mono.Unix.Native.Syscall.close(ffd);
				throw new Exception("Error calling LOOP_SET_FD ioctl() : "+retCode);
            }
			retCode = UnsafeNativeMethods.ioctl(fd, LOOP_SET_STATUS, ref loopinfo);
            if (retCode < 0){
                int ro = 0;
                UnsafeNativeMethods.ioctl(fd, LOOP_CLR_FD, ref ro);
                Mono.Unix.Native.Syscall.close(fd);
                Mono.Unix.Native.Syscall.close(ffd);
                throw new Exception("Error calling LOOP_SET_STATUS ioctl() : "+retCode);
            }
        ...
        public void RemoveLoop(string device){
            int fd;

            if((fd = Mono.Unix.Native.Syscall.open(device, Mono.Unix.Native.OpenFlags.O_RDONLY)) < 0){
                throw new Exception("Error removing loop device, cannot open '"+device+"': " + Mono.Unix.Native.Syscall.strerror(Mono.Unix.Native.Syscall.GetLastError()));
            }

            int r = 0;
            if(UnsafeNativeMethods.ioctl(fd, LOOP_CLR_FD, ref r) < 0) {
                //perror_msg("ioctl: LOOP_CLR_FD");
                throw new Exception("Error removing loop device, ioctl: LOOP_CLR_FD, Error description: " + Mono.Unix.Native.Syscall.strerror(Mono.Unix.Native.Syscall.GetLastError()));
        ...
        static class UnsafeNativeMethods{
            ...
            [System.Runtime.InteropServices.DllImport("libc", SetLastError = true)]
            private static extern int mount(string source, string target, string filesystemtype, UIntPtr mountflags, /*System.IntPtr*/string data);
            //int mount(const char *source, const char *target, const char *filesystemtype, ulong mountflags, const void *data);


            public static int mount(string source, string target, string filesystemtype, MountFlags mountflags, /*System.IntPtr*/string data){
        ...
        public string GetLoop(string diskPath, string mountPoint){

            const bool SUCCESS = true;
            string strLoopDeviceToUse = find_unused_loop_device();
            string strMountPoint = mountPoint;

            int ro = 0;
            int ret = set_loop(strLoopDeviceToUse, diskPath, 0, ref ro);

			return strLoopDeviceToUse;
        ...
        private static string loopfile_from_sysfs(string device){
            string res = null;
