...
namespace Node{
	...
	/// It will process chunks building and transferring following the provided level of parallelism.
	/// Parallelism will directly impact memory usage:
	...
	public class BackupManager{
		...
		internal Backup Backup{get{return backup;}}

		public delegate void BackupDoneHandler(Backup backup);
		public event BackupDoneHandler BackupDoneEvent;

		public delegate void StorageNeededHandler(PeerSession s, int parallelism, bool isIndex,  bool alternateRequest);
		public event StorageNeededHandler StorageNeeded;

		// An index db (backup index of dedup index) is missing. Event must be suscribed and request for a db backup sent to Hub
		public delegate void IndexNeededHandler(long taskId, bool IsDedupDBRequest);
		public event IndexNeededHandler IndexNeeded;
		...
		/// Task identifier.
		/// </param>
		public BackupManager(Backup b){

			backup = b;
			int backingDevicesCount = 0;
			megaQueue = new Queue<Queue<BackupRootDrive>>();

			if(backup.BackupSet.Parallelism.Kind == ParallelismLevel.Disk){
			...
			else if(backup.BackupSet.Parallelism.Kind == ParallelismLevel.FS){
			...
			if(backup.BackupSet.Parallelism.Value == 0){ // "auto" mode, set to the number of backing blockdevices (physical disk or partition)
				...
				realParallelism = Math.Min((int)backup.BackupSet.Parallelism.Value, backingDevicesCount) ;

			Logger.Append(Severity.INFO, "Requested parallelism: "+this.backup.BackupSet.Parallelism.ToString()+", real: "+realParallelism);
			...
			consumers = new List<System.Threading.Tasks.Task>((int)b.BackupSet.Parallelism.Value);
		}
		
		~BackupManager(){
			Logger.Append(Severity.TRIVIA, "<TRACE> BackupManager destroyed.");	
		}
		...
		internal void Run(){
			...
			this.StorageNeeded(new PeerSession{TaskId = backup.Id, Id = -1}, realParallelism, false, false);
		...
		private void Consume(Session s/*, int budget*/){
			...
			while((!myFeed.IsCompleted)  && (s.Budget >0)){
				...
				try{
					...
					if(chunk.SentEvent != null) chunk.SentEvent.Dispose();
				}
		...
		private void AfterTask(System.Threading.Tasks.Task task, Session s){
			lock(consumers){ // not frequently called, so a global lock is perfectly fine here
				...
				for(int i= consumers.Count-1; i>=0; i--){
					Console.WriteLine ("################### inspecting task "+consumers[i].Id+", completed="+consumers[i].IsCompleted+", faulted="+consumers[i].IsFaulted+", cancelled="+consumers[i].IsCanceled);
					if(consumers[i].IsCanceled || consumers[i].IsCompleted){
						consumers[i].Dispose();
			...
			if(chunksFeeds.Count >0 && ! cancellationTokenSource.IsCancellationRequested){
				Logger.Append(Severity.DEBUG, "---------------------------------Asking for another storage session because of expired budget with storage node #"+s.ToNode.Id+", current chunks feed queue count : "+chunksFeeds.Count);
				this.StorageNeeded(s, 1, false, false);
				return;
			}
		...
		private void UnexpectedError(System.Threading.Tasks.Task task){
			var aggException = task.Exception.Flatten();
         	foreach(var exception in aggException.InnerExceptions){
				Logger.Append(Severity.CRITICAL, "Unexpected error while processing backup task "+this.backup.Id+" : "+exception.ToString());
				if(exception.InnerException != null)
					Logger.Append(Severity.CRITICAL, "Inner Exception :  "+this.backup.Id+" : "+exception.InnerException.ToString());
			...
			lock(processingChunks){
				for(int i = processingChunks.Count-1; i>=0; i--){
					if(processingChunks[i].Name == chunkName){
						...
						backup.TotalChunks ++;
						backup.TotalItems += processingChunks[i].Items.Count;
						
						indexerChunksFeed.Add(processingChunks[i], cancellationTokenSource.Token);
						processingChunks.RemoveAt(i);
						Logger.Append (Severity.TRIVIA, "Sent and indexed chunk "+chunkName);
					}
				}
				// to remove?
		...
		private void DoIndex(){
			while( !indexerChunksFeed.IsCompleted /*|| !cancellationTokenSource.IsCancellationRequested*/){
				try{
					...
					Logger.Append(Severity.TRIVIA, "Added chunk "+toBeIndexed.Name+" to index");
				}
				catch(Exception e){
					if(e is OperationCanceledException)
						Logger.Append(Severity.TRIVIA, "Indexer has been manually cancelled on purpose, stopping...");
					else if(e is InvalidOperationException)
						Logger.Append(Severity.TRIVIA, "Indexer : no more chunks to index ("+e.Message+"): "+e.ToString());
					else{
		...
		private BChunk BuildIndexChunk(){
			...
			try{
				...
				StorageNeeded(new PeerSession{TaskId = backup.Id}, 1, true, false);
