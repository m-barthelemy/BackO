...
Ext.define('Extensible.calendar.form.EventWindow', {
	...
	Ext.create('Extensible.calendar.form.field.ReminderCombo', {
	...
	{
		xtype:'boxselect',
		fieldLabel: this.attendeesLabelText,

						//id:'bossList-'+record.get('Id'),
						width:500,
						height:60,
						resizable: true,
						scroll:false,
						...
						hideTrigger:false,
						name: 'to',
						store: favPeople, // !!TODO : change!! must not use people store, but instead search sxerver
						queryParam:'Name',
						queryMode:'local',
						typeAhead:true,
						mode: 'local',
						displayField: 'Name',
						emptyText:'Select participants',
						placeHolder:'Select participants',
						delimiter:' ',
						multiSelect:true,
					    multiSelectItemTpl: 
					    	'<tpl for=".">'
					    	+'<li class="x-boxselect-item" style="border:1px solid grey; height:18px; padding-left: 1px;">'
					    		+'{FirstName} {LastName}'
	            		...
	            		listConfig: {
						      "tpl": [
						         "<ul><tpl for=\".\">",
						         "<li role=\"option\" class=\"x-boundlist-item\" style=\" padding-left: 3px;\">{FirstName} {LastName}</li>",
						         "</tpl></ul>"
						      ],
						 },
						 listeners:{
						 	/*afterrender:function(thisobj, eopts){
						 		thisobj.suspendEvents();
						 		var valz = [];
						 		var resTokens = record.get('BossId');
						 		if(resTokens != ''){
									for (var i = 0; i < resTokens.length; i++) {
										var tagRecord =  peopleCache.findRecord('Id', resTokens[i]); // tagStore.getById(parseInt(resTokens[i]));
										if(tagRecord != null)
											valz.push(tagRecord.get('FirstName')+' '+tagRecord.get('LastName'));
									}
							 		this.setValue(valz);
							 		initialValues = valz;
						 		}
						 		thisobj.resumeEvents();
						 	},*/
						 	/*change:function(thisobj, newValue, oldValue, eOpts){
						 		record.set('BossId', newValue);
						 	}*/
						 } // end listeners
	        		//}
	        	}, // end participants combo
	...
	Ext.create('Extensible.form.recurrence.Fieldset', {
